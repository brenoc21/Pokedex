...
function Home() {
  ...
  useEffect(() => {
    if (search != "") {
      setRenderData(
        pokemon &&
          pokemon.map((item) =>
            item.filter((poke) => poke.name.includes(search))
          )
      );
    } else {
  ...
  return (
    <Cardbox>
      ...
      <ShinyButton onClick={() => setShiny(!shiny)}>
        {" "}
        {shiny ? (
          <AiFillStar className="svg" />
        ) : (
          <AiOutlineStar className="svg" />
        )}{" "}
      </ShinyButton>
      <ShinyButton onClick={() => setOld(!old)}>
        {" "}
        {old ? (
          <FaHourglass className="svg" />
        ) : (
          <FaRegHourglass className="svg" />
        )}{" "}
      </ShinyButton>
      <Container>
